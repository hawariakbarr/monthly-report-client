...
userForm.addEventListener("submit", function(e){  
  ...
  $.ajax({
    url: `${urlData}report/add-opd-link`,
    type: 'POST',
    ...
    success: function (result) {
        ...
        if(result.error)
        {
          ...
          document.getElementById("overlay").setAttribute("hidden", false);      
          iziToast.success({
            title: 'Opd link Berhasil Ditambahkan',
            message: `Opd link dengan prtg-id ${result.data.prtg_id} Berhasil Disimpan`,
            position: 'topRight'
          })
          $('#addModal').modal('hide');
          setTimeout(() => {
...
async function getDataTable(urlData){
    await $.ajax({
      url: `${urlData}report/get-opd-link`,
      type: 'GET',
      headers: {
      ...
      success: function (result) {
        ...
        <div class="table-responsive">
          <table class="table table-striped" id="table-1">
            <thead>
              <tr>
                <th style="width:250px">Nama OPD</th>
                <th>PRTG ID</th>
                <th>ISP</th>
                <th>Bandwith</th>
...
function openEditModal(id){
    $.ajax({
      url: `${urlData}report/get-opd-link/` + id,
      type: 'GET',
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        "Authorization": getCookie("session")
          ...
          `
            ...
            <div class="card">        
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="listopdEdit" name="opd name" required placholder="--Select Opd--">
          ...
          $.ajax({
            ...
            success: function (result) {      
              if(result.data.length > 0)
                result.data.forEach((element, index) => {
                  $('#listopdEdit').append(`<option value="${element.id}">${element.name}</option>`);
                }); 
            },
            complete: function () {
              document.getElementById("overlay").setAttribute("hidden", false);
...
function editData(id){
    $('#editFormData').on("submit", function(e){
        ...
        var listOpdEdit = document.getElementById('listopdEdit').value
        ...
        const data = { 
          ...
          opd_id: parseInt(listOpdEdit),
          isp_id: parseInt(listIspEdit),
          band_id: parseInt(listBandwithEdit)
        };

        $.ajax({
        url: `${urlData}report/update-opd-link/` + id,
        type: 'PUT',
        headers: {
            "Authorization": getCookie("session")
        },
...
async function deleteData(id){
    ...
    if (result) 
        $.ajax({
        url: `${urlData}report/delete-opd-link/` + id,
        type: 'DELETE',
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": getCookie("session")
...
async function getDataUptd(id){
  await $.ajax({
    ...
    beforeSend: function () {
        document.getElementById("overlay").removeAttribute("hidden");
        $('#listUptd').prop("disabled", false); // Element(s) are now enabled.
    },
    success: function (result) {       
