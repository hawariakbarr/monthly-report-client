...
const userForm = document.getElementById("opdFormData");
userForm.addEventListener("submit", function(e){  
  e.preventDefault();
  var opdName = document.getElementById('opdName').value
  var opdAddress = document.getElementById('opdAddress').value
  var opdPic = document.getElementById('opdPic').value
  var opdPhone =  document.getElementById('opdPhone').value
  ...
  const data = { 
    ...
    address: opdAddress,
    pic: opdPic,
    phone_number: opdPhone
  };

  $.ajax({
    url: `${urlData}report/add-opd`,
    type: 'POST',
...
async function getDatatableOpd(urlData){
    await $.ajax({
      url: `${urlData}report/get-opd-all`,
      type: 'GET',
      headers: {
          ...
          listData += `
            <tr>
              ...
              <td>
                ...
                <button class="btn btn-danger btn-sm btn-icon" data-toggle="tooltip" title="Delete" onClick="deleteOpd(${element.id})" ><i class="fas fa-trash fa-sm" style=" color:white"></i></button>
              </td>
            </tr>
          `
        }); 
          ...
          `
            ...
            <div class="card">        
              <div class="card-body">
                ...
                <div class="form-group">
                  <label>PIC</label>
                  <input type="text" class="form-control" id="picEdit" required="" autocomplete="off" placeholder="Masukan Nama PIC" Value="${result.data.pic}">
                </div>
                <div class="form-group">
...
function editData(id){
    $('#editFormData').on("submit", function(e){
        ...
        var name = document.getElementById('opdnameEdit').value
        ...
        $.ajax({
        url: `${urlData}report/update-opd/` + id,
        type: 'PUT',
        headers: {
            "Authorization": getCookie("session")
        },
...
async function deleteOpd(id){
    ...
    if (result) 
        $.ajax({
        url: `${urlData}report/delete-opd/` + id,
        type: 'DELETE',
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": getCookie("session")
